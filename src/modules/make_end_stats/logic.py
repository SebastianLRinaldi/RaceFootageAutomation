...
class OverlayWorker(QThread):
    ...
    def run(self):
        try:
            self.logic.make_stats_overlay()  # call the instance method
            self.finished.emit()
...
class Logic(QObject, Blueprint):
    ...
    def __init__(self, component):
        ...
        self.rendered_name = f"stats_Overlay.mp4"
        self.asset_name = f"stats_temp.mp4"
        ...
        self.settings_handler = SettingsHandler(SETTINGS_FIELDS, target=self, app="make_end_stats")

    def generate_overlay(self):
        self.generate_button.setEnabled(False)
        self.status_label.setText("Generating Stats Overlay...")
    ...
    def create_stats_section(self, lap_number, temp_dir):
        """
        ...
        try:
            for frame in range(frame_count):
                ...
                # draw.multiline_text((0,0), stats, font=font, fill=text_color)

                # Calculate bounding box of multiline text
                bbox = draw.multiline_textbbox((0, 0), stats, font=font)
                ...
                y = (self.FRAME_HEIGHT - text_height) // 2

                # Draw multiline text centered
                draw.multiline_text((x, y), stats, font=font, fill=text_color)
    ...
    def make_stats_overlay(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if render_single:
                lap_video = self.create_stats_section( 1, temp_dir)
                lap_videos.append(lap_video)
            else:
                with ThreadPoolExecutor() as executor:
                    futures = {
                                executor.submit(self.create_stats_section, i , temp_dir): i
