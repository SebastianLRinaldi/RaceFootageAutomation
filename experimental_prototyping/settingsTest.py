...
class OverlaySettingsWidget(QWidget):
    def __init__(self, index):
        super().__init__()
        label = QLabel(f"Settings for Overlay {index+1}")
        label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout = QVBoxLayout()
        layout.addWidget(label)
        # Add actual settings widgets here per overlay
        self.setLayout(layout)

class OverlayTab(QWidget):
    def __init__(self, overlay_count=7):
        super().__init__()
        self.overlay_list = QListWidget()
        for i in range(overlay_count):
            self.overlay_list.addItem(f"Overlay {i+1}")
        self.overlay_list.setCurrentRow(0)

        self.preview = QLabel("Overlay Preview")
        self.preview.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.preview.setStyleSheet("background-color: #ddd; border: 1px solid #ccc;")
        self.preview.setFixedHeight(150)

        self.edit_btn = QPushButton("Edit Overlay")

        # Stacked widget for overlay settings
        self.settings_stack = QStackedWidget()
        for i in range(overlay_count):
            self.settings_stack.addWidget(OverlaySettingsWidget(i))

        # Layout right side: preview + button + settings stack
        ...
        main_layout.addWidget(self.overlay_list, 2)
        main_layout.addLayout(right_layout, 5)
        self.setLayout(main_layout)

        # Connect list selection to stacked widget index
        self.overlay_list.currentRowChanged.connect(self.settings_stack.setCurrentIndex)

if __name__ == "__main__":
    app = QApplication(sys.argv)
    w = OverlayTab()
    w.setWindowTitle("Overlay Tab with Multiple Settings")
    w.resize(700, 400)
    w.show()
    sys.exit(app.exec())
