...
#     # Draw vertical lines for columns (headers + grid lines)
#     x_offset = x
#     for width in COL_WIDTHS:
#         x_offset += width
#         cv2.line(img, (x_offset, y), (x_offset, y + total_rows * row_h), WHITE, 1)
...
#     # Draw data rows
#     for row_idx, lap_time in enumerate(lap_times):
#         top = y + row_h * (row_idx + 1)
#         cv2.rectangle(img, (x, top), (x + TABLE_WIDTH, top + row_h), WHITE, 1)

#         x_offset = x
#         # Lap number
#         lap_str = str(row_idx + 1)
#         # Time
#         time_str = f"{lap_time:.3f}" if lap_time is not None else "N/A"
#         # Sector (dummy example: 40% of lap_time)
#         sector_str = f"{lap_time * 0.4:.3f}" if lap_time is not None else "N/A"

#         row_cells = [lap_str, time_str, sector_str]
#         for col_idx, width in enumerate(COL_WIDTHS):
#             center_x = x_offset + width // 2
#             cell_text = row_cells[col_idx] if col_idx < len(row_cells) else ""
#             img = draw_centered_text_pil(img, cell_text, center_x, top + row_h // 2, font_size_row, WHITE)
#             x_offset += width
...
FONTPATH = "C:\\Users\\epics\\AppData\\Local\\Microsoft\\Windows\\Fonts\\NIS-Heisei-Mincho-W9-Condensed.TTF"
IMG_FOLDER = "lap_tables_test_2"
OUTPUT_VIDEO = "lap_table_video_test_multi_column.mp4"
FFMPEG_INPUT_FILE = "ffmpeg_input.txt"
...
if __name__ == "__main__":
    ...
    with open(FFMPEG_INPUT_FILE, "w") as f:
        ...
        for i in tqdm(range(0, len(LAP_TIMES)), desc="Writing ffmpeg input"):
            t = LAP_TIMES[i][0]
            duration = t if t is not None else 5
            img_file = f"lap_table_{i+1:02}.png"  # notice +1 since 00 is header only
            f.write(f"file '{os.path.join(IMG_FOLDER, img_file)}'\n")
            f.write(f"duration {duration}\n")
