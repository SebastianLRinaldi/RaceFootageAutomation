...
OUTPUT_VIDEO = "lap_table_video_count_down_times_test2.mp4"
FFMPEG_INPUT_FILE = "ffmpeg_input.txt"

START_DURATION = 5
LAST_HOLD_DURATION = 5
FPS = 30  # target frames per second for video

# Precompute cumulative lap times for timing sync
cum_lap_times = [sum(LAP_TIMES[:i+1]) for i in range(len(LAP_TIMES))]
...
def get_laps_to_display(lap_times, frame_idx, fps):
    elapsed = frame_idx / fps
    laps = []
    for i, lap_end_time in enumerate(cum_lap_times):
        if elapsed >= lap_end_time:
            laps.append(lap_times[i])
        else:
            prev_cum = cum_lap_times[i-1] if i > 0 else 0
            partial_alpha = (elapsed - prev_cum) / (lap_end_time - prev_cum)
            partial_alpha = max(0, min(partial_alpha, 1))
            partial_time = lap_times[i] * partial_alpha
            laps.append(partial_time)
            break
    return laps

def make_table_frame(frame_idx):
    img = np.zeros((FRAME_HEIGHT, FRAME_WIDTH, 3), dtype=np.uint8)
    laps_to_draw = get_laps_to_display(LAP_TIMES, frame_idx, FPS)
    return draw_table(img, TABLE_X, TABLE_Y, laps_to_draw)
...
if __name__ == "__main__":
    ...
    # Blank frame for start
    blank_img = make_blank_frame()
    cv2.imwrite(os.path.join(IMG_FOLDER, "blank.png"), blank_img)

    # Header only frame
    header_img = draw_table(
        img=np.zeros((FRAME_HEIGHT, FRAME_WIDTH, 3), dtype=np.uint8),
        x=TABLE_X,
        y=TABLE_Y,
        lap_times=[]
    )
    cv2.imwrite(f"{IMG_FOLDER}/lap_table_00.png", header_img)
    print(f"Saved: {IMG_FOLDER}/lap_table_00.png")

    if not LAP_TIMES:
        raise ValueError("LAP_TIMES is empty. Nothing to render.")

    # Generate frames for total video duration + some margin
    total_duration = sum(LAP_TIMES) + LAST_HOLD_DURATION
    total_frames = int(total_duration * FPS) + 1

    for frame_idx in tqdm(range(total_frames), desc="Generating frames"):
        img = make_table_frame(frame_idx)
        filename = f"{IMG_FOLDER}/lap_table_{frame_idx:05}.png"
        cv2.imwrite(filename, img)

    # Write ffmpeg input file with each frame as a single image (constant frame rate)
    with open(FFMPEG_INPUT_FILE, "w") as f:
        for frame_idx in range(total_frames):
            img_file = f"lap_table_{frame_idx:05}.png"
            f.write(f"file '{os.path.join(IMG_FOLDER, img_file)}'\n")
            f.write("duration 0.0333\n")  # ~1/FPS seconds per frame

        # Append last frame longer to hold
        f.write(f"file '{os.path.join(IMG_FOLDER, img_file)}'\n")
